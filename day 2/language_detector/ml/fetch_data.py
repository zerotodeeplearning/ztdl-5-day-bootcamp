...
pages = {
    'ar': 'http://ar.wikipedia.org/wiki/%D9%88%D9%8A%D9%83%D9%8A%D8%A8%D9%8A%D8%AF%D9%8A%D8%A7',
    'de': 'http://de.wikipedia.org/wiki/Wikipedia',
    'en': 'http://en.wikipedia.org/wiki/Wikipedia',
    ...
    'ru': 'http://ru.wikipedia.org/wiki/%D0%92%D0%B8%D0%BA%D0%B8%D0%BF%D0%B5%D0%B4%D0%B8%D1%8F',
#    u'zh': u'http://zh.wikipedia.org/wiki/Wikipedia',
}

html_folder = 'wikidata/html'
text_folder = 'wikidata/paragraphs'
short_text_folder = 'wikidata/short_paragraphs'
n_words_per_short_text = 5
n_chars_per_short_text = 30


if not os.path.exists(html_folder):
    os.makedirs(html_folder)

for lang, page in list(pages.items()):
    ...
    for p in tree.findall('//p'):
        ...
        # FIXME: whitespace tokenizing does not work on chinese and japanese
            words = list(content)
            n_groups = len(words) / n_chars_per_short_text
        else:
            words = content.split()
